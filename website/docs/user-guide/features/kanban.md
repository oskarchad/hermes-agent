...
- **Exact origin-session delivery remains the first choice.** The session that created the task owns delivery while it is live — the same `platform="tui"` subscription that posts the terminal event back into the running session.
- **When that origin session is absent or finalized, the Captain inbox takes over.** The next active TUI/Desktop session bound to the **same** normalized profile may claim the report — exactly one of them wins, no matter how many same-profile sessions are polling.
...
- `pending` → `leased` → `acked`. A poll atomically **leases** a pending row under an opaque token before it renders anything.
- The lease is **acked only after the synthetic status turn is accepted** by the session. If that turn is rejected or the dispatch fails, the lease is **released** back to `pending` for a later retry.
- Each lease carries an expiry, so a process that crashes between lease and ack strands the row only until the lease expires; the first poll after a **restart reclaims** the expired lease and delivers exactly once.
...
**Bounded, privacy-safe payload.** A Captain report carries only task/event-derived fields: board/task ids, title, assignee, event kind, and the **first line** of the worker's handoff. It never includes the task body, conversation history, arbitrary metadata, or raw tool/spawn errors; credential-shaped text is force-redacted at this boundary even when global output redaction is disabled.
