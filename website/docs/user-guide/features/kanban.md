...
- `pending` → `leased` → `acked`. A poll rechecks origin liveness and tenant eligibility in the **same write transaction** that leases the row under an opaque token, so an origin that refreshes between candidate read and lease cannot be stolen by a sibling gateway.
- After a successful synthetic turn, Hermes first persists the transcript and commits the inbox acknowledgement, then emits the visible assistant `message.complete` with a stable event-derived completion ID. TUI and Desktop deduplicate that ID defensively.
- If the transport dies after accepting the completion frame, no pending inbox row remains to replay. If it dies just before the frame, reconnect hydration reads the already-durable assistant row. Rejection, interruption, or model failure before the transcript/ack commit releases the lease for retry.
- Each lease carries an expiry, so a process that crashes before settlement strands the row only until the lease expires; the first eligible poll after restart can reclaim it.
...
**Bounded, privacy-safe payload.** A Captain report carries only task/event-derived fields: board/task ids, title, assignee, event kind, and the **first line** of the worker's handoff. One poll applies a single row cap across all boards plus a global UTF-8 byte cap, so adding boards cannot multiply one session's prompt. The report never includes the task body, conversation history, arbitrary metadata, or raw tool/spawn errors; credential-shaped text is force-redacted at this boundary even when global output redaction is disabled.
