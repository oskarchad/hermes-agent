...
- **Same-card review:** call `kanban_request_review(summary=..., metadata=..., reviewer=...)`. The task enters `review` without touching block recurrence accounting. When `reviewer` names a different profile, task-forced implementation skills are recorded on the handoff and removed from the review phase; the dispatcher loads only the bundled `sdlc-review` skill on top of that reviewer's own profile skills. Assigning a different reviewer in `review`, or while the card is waiting to resume review in `blocked`, dependency-wait `todo`, or `scheduled`, performs the same phase handoff atomically. CLI/dashboard two-step routing therefore cannot leak implementation-only skill flags through a later unblock or dependency recompute. On upgrade, the first claim of an already-persisted pre-fix cross-profile review performs this same reconciliation atomically before spawn and records the recovered provenance in the audit trail. Ambiguous or malformed non-empty legacy handoffs stay parked, do not reserve host capacity from healthy ready work, and emit one sanitized `review_dispatch_refused` audit event instead of spawning unsafe flags; truly old reviews with a provably empty `NULL`/`[]` skillset remain routable. `kanban_request_changes` restores the exact implementation skillset before routing repair back. Omitting `reviewer` (or naming the same profile) is the explicit contract for sharing task-forced skills across both phases. The reviewer approves with `kanban_complete`, calls `kanban_request_changes(reason=...)` to close the review run and route the task back to its original implementer, or blocks only for a genuine external escalation.
