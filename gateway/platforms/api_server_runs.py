...
async def _handle_runs(
    ...
    *,
    ...
    # Local: the protected-approval bridge resolves an operator approval to
    # exactly one live run by session identity. Recorded after the
    # precedence rule above so the map holds the effective session_id.
...
def _sweep_orphaned_runs_once(self, now: Optional[float] = None) -> None:
    ...
    for run_id in stale_statuses:
        ...
        # Local: the protected-approval bridge keys live runs by operator
        # session identity, and no terminal-status path clears that map, so
        # the status sweep is its only release point.
        self._run_operator_sessions.pop(run_id, None)
        self._release_run_owner_if_forgotten(run_id)
