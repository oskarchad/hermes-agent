...
class TestSessionKeyHeader:
    ...
    @pytest.mark.asyncio
    async def test_authenticated_webui_turn_cannot_approve_protected_write_by_text(
        self, auth_adapter
    ):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                mock_run.return_value = (
                    {"final_response": "normal model turn", "messages": [], "api_calls": 1},
        ...
        assert data["choices"][0]["message"]["content"] == "normal model turn"
