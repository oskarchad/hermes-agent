...
def test_run_prompt_submit_requires_explicit_persistence_before_success_emit(
    ...
    try:
        ...
        # Cold AIAgent imports can exceed three seconds under the per-file
        # parallel runner; completion is event-driven, so keep a generous
        # deadlock bound rather than asserting host speed.
        assert settled.wait(10.0)
        session["_run_thread"].join(timeout=10.0)
    finally:
