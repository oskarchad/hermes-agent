...
async def test_forced_headless_oauth_eof_does_not_retry_authorization(monkeypatch):
    ...
    monkeypatch.setattr("tools.mcp_tool_common._jittered", lambda _seconds: 0.0)
    monkeypatch.setattr(mcp_tool, "_jittered", lambda _seconds: 0.0, raising=False)
...
async def test_state_mismatch_is_redacted_before_sdk_and_application_logs(
    ...
    for error in caught_errors:
        assert str(error) == "OAuth state parameter mismatch"
