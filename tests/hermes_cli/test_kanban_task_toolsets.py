...
def test_default_spawn_uses_exact_task_toolsets_in_worker_and_reviewer_phases(
    ...
    _allow_task_toolsets(monkeypatch)
    monkeypatch.setattr("hermes_cli.kanban_db_dispatch._resolve_hermes_argv", lambda: ["hermes"])
    monkeypatch.setattr("hermes_cli.kanban_db_dispatch._resolve_worker_cli_toolsets",
        lambda _home: [
            "web",
            "terminal",
            "context7",
...
""".lstrip(),
    ...
    _default_spawn(legacy_task, str(workspace))
    _default_spawn(explicit_task, str(workspace))
...
def test_bounded_worker_toolsets_reduce_real_model_tool_schema(
    ...
    unrelated = {
        ...
        "delegate_task",
        "session_search",
        "todo_list" if "todo_list" in broad_names else "todo",
    }
