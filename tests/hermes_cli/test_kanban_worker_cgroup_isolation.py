...
def _prepare_spawn(monkeypatch, tmp_path):
    ...
    patch_dir = root / "profiles" / "patch"
    patch_dir.mkdir(parents=True)
    patch_dir.joinpath("config.yaml").write_text("{}\n", encoding="utf-8")
...
def test_timeout_uses_scope_aware_cleanup_and_records_sanitized_target(
    ...
    try:
        ...
        assert kbd.enforce_max_runtime(conn) == [task_id]
...
def test_scope_unavailable_is_spawn_failure_not_worker_crash(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(process_registry, "_is_supervised_gateway_process", lambda: True)
    monkeypatch.setattr(process_registry, "_systemd_run_user_scope_available", lambda: False)
    monkeypatch.setattr(kbw, "resolve_workspace", lambda _task, board=None: workspace)

    conn = kbc.connect()
    try:
        task_id = kb.create_task(conn, title="isolated dispatch", assignee="patch")
        result = kbd.dispatch_once(conn, failure_limit=2)
...
def test_dispatch_stops_exact_spawn_when_pid_persistence_cas_loses(
    ...
    try:
        ...
        result = kbd.dispatch_once(conn, spawn_fn=spawn_then_lose_claim)
...
def test_dispatch_pre_pid_manual_recovery_preserves_original_owner(
    ...
    try:
        ...
        result = kbd.dispatch_once(conn, spawn_fn=spawn_during_recovery)
...
def test_pid_cas_loss_cleanup_failure_does_not_mutate_successor(
    ...
    try:
        ...
        result = kbd.dispatch_once(conn, spawn_fn=spawn_then_replace_owner)
...
def test_implementation_to_review_waits_for_loaded_source_scope_then_claims(
    ...
    try:
        ...
        deferred = kbd.dispatch_once(conn, spawn_fn=spawn_review, failure_limit=1)
        ...
        claimed = kbd.dispatch_once(conn, spawn_fn=spawn_review, failure_limit=1)
...
def test_review_to_repair_waits_for_live_source_process_then_claims(
    ...
    try:
        ...
        deferred = kbd.dispatch_once(conn, spawn_fn=spawn_repair, failure_limit=1)
        ...
        live_pids.clear()
        claimed = kbd.dispatch_once(conn, spawn_fn=spawn_repair, failure_limit=1)
...
def test_genuine_spawn_failure_still_trips_failure_budget(monkeypatch, tmp_path):
    ...
    try:
        ...
        result = kbd.dispatch_once(conn, spawn_fn=fail_spawn, failure_limit=1)
...
def _install_successor_between_snapshot_and_scope_lookup(kb, monkeypatch, conn, task_id, lock, new_pid):
    ...
    def racing(conn_, tid, run_id=None):
        if not state["swapped"]:
            ...
            state["before_task"] = tuple(
                conn_.execute(
                    "SELECT status, current_run_id, worker_pid, claim_lock, "
                    "claim_expires, last_heartbeat_at FROM tasks WHERE id = ?",
                    (tid,),
                ).fetchone()
            )
            state["before_run"] = tuple(
                conn_.execute(
                    "SELECT status, ended_at, claim_expires, worker_pid FROM task_runs "
                    "WHERE id = ?",
...
@pytest.mark.parametrize(
    "cleanup_path",
    ["manual_reclaim", "expired_claim", "max_runtime", "stale_heartbeat"],
)
def test_failed_cleanup_race_never_defers_or_mutates_successor(
    monkeypatch, tmp_path, cleanup_path
):
    """Every failed-cleanup path must fence its defer to the snapshotted run."""
    from hermes_cli import kanban_db as kb
    ...
    stopped = []
    monkeypatch.setattr(
        kb, "_stop_kanban_worker_scope", lambda unit: stopped.append(unit) or False
    )
    ...
    try:
        task_id = kb.create_task(
            conn,
            title=f"{cleanup_path} failed cleanup race",
            assignee="patch",
            max_runtime_seconds=1 if cleanup_path == "max_runtime" else None,
        )
        lock = kb._dispatcher_claim_lock(conn, task_id)
        old_run = kb.claim_task(conn, task_id, claimer=lock)
        assert old_run is not None
        ...
        now = int(time.time())
        if cleanup_path == "expired_claim":
            conn.execute(
                "UPDATE tasks SET claim_expires = ? WHERE id = ?",
                (now - 600, task_id),
            )
        elif cleanup_path in {"max_runtime", "stale_heartbeat"}:
            conn.execute(
                "UPDATE task_runs SET started_at = ? WHERE id = ?",
                (now - 7200, old_run.current_run_id),
            )
            if cleanup_path == "stale_heartbeat":
                conn.execute(
        ...
        if cleanup_path == "manual_reclaim":
            assert kb.reclaim_task(conn, task_id, signal_fn=lambda *_a: None) is False
        elif cleanup_path == "expired_claim":
            assert kb.release_stale_claims(conn, signal_fn=lambda *_a: None) == 0
        elif cleanup_path == "max_runtime":
            assert kbd.enforce_max_runtime(conn, signal_fn=lambda *_a: None) == []
        else:
        ...
        assert successor is not None
        assert stopped == [
            kb._kanban_worker_scope_unit(task_id, old_run.current_run_id)
        ]
        after_task = tuple(
            conn.execute(
                "SELECT status, current_run_id, worker_pid, claim_lock, "
        ...
        assert after_task == state["before_task"]
        assert after_run == state["before_run"]
        assert after_events == state["before_events"]
    finally:
        conn.close()


@pytest.mark.linux_only
def test_handoff_guard_ignores_foreign_persisted_scope(monkeypatch, tmp_path):
    from hermes_cli import kanban_db as kb
    ...
    conn = kbc.connect()
    try:
        task_id = kb.create_task(conn, title="foreign handoff scope", assignee="patch")
        implementation = kb.claim_task(conn, task_id)
        assert implementation is not None
        expected = kb._kanban_worker_scope_unit(task_id, implementation.current_run_id)
        foreign = "hermes-worker-kanban-other-run-999.scope"
        assert kb._set_worker_pid(
            conn,
            task_id,
            kb._SpawnedWorkerPid(
        ...
        assert kb.request_review(
            conn,
            task_id,
            summary="ready for exact-scope review",
            expected_run_id=implementation.current_run_id,
        )
        probed = []
        monkeypatch.setattr(
            kb,
            "_systemd_worker_scope_unloaded",
            lambda unit: probed.append(unit) or unit == foreign,
        )
...
@pytest.mark.linux_only
def test_handoff_guard_fails_closed_without_exact_scope(monkeypatch, tmp_path):
    from hermes_cli import kanban_db as kb
    ...
    conn = kbc.connect()
    try:
        task_id = kb.create_task(conn, title="unresolved handoff scope", assignee="patch")
        implementation = kb.claim_task(conn, task_id)
        assert implementation is not None
        ...
        assert kb.request_review(
            conn,
            task_id,
            summary="ready after exact scope teardown",
            expected_run_id=implementation.current_run_id,
        )
        probed = []
        monkeypatch.setattr(kb, "_worker_scope_unit", lambda *_args, **_kwargs: None)
        monkeypatch.setattr(
            kb,
            "_systemd_worker_scope_unloaded",
