...
_TASK_OPTIONAL_COLUMNS = (
    ...
    "current_step_key", "max_retries", "session_id", "completion_contract", "enabled_toolsets",
)
...
-- Captain reporting registry: maps a task to the profile (and optional
-- origin TUI/Desktop session key) that acts as Captain for it.
...
def _resolve_captain_ownership(
    ...
    *,
    ...
    """Resolve one immutable Captain owner (profile + origin) for a root or descendant task.

    Tenant is deliberately NOT decided here: ``initial_task_state`` owns that policy
    (explicit wins, else first non-null parent), and the registry row copies the
    resolved ``tasks.tenant``. Two writers disagreeing turned an explicit child
    tenant into a hard error.
    """
    ...
    if parent_rows:
        ...
        return next(iter(profiles)), next(iter(origins), None)

    profile = requested_profile or resolve_captain_profile()
    return _normalize_captain_profile(profile), requested_origin
...
def create_task(
    ...
    captain_profile, captain_origin_session_key = _resolve_captain_ownership(
        conn,
        parents,
...
_PLUGIN_COMPAT_LAZY = {
    ...
    '_dispatcher_claim_lock': ('hermes_cli.kanban_db_connect', '_dispatcher_claim_lock'),
