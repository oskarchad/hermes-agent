...
def _user_deny_block(command: str) -> dict | None:
    ...
    ``approvals.deny_commands`` (ADR 0003) is the opt-in command-aware selector layer with the
    same never-bypassable standing; it runs first so a quoted ``merge``/``push`` inside a PR body
    cannot be denied by the whole-text glob below."""
